...
@interface CardView ()<DragCardItemDelegate>
...
{
    CardView * cardView = [[CardView alloc]initWithFrame:superView.frame];
...
-(void)initCards{
    for (int i = 0; i<CARD_NUM; i++) {
        ...
        CardViewItem * card = [[CardViewItem alloc]initWithFrame:CGRectMake(SCREEN_WIDTH +CARD_WIDTH, self.center.y- CARD_HEIGHT/2, CARD_WIDTH, CARD_HEIGHT)];
...
-(void)loadAllCards{
    ...
    for (int i=0; i<self.disPlayCardArr.count; i++) {
        ...
        CardViewItem *card=self.disPlayCardArr[i];
    ...
    for (int i=0; i< self.disPlayCardArr.count ;i++) {
        ...
        [UIView animateKeyframesWithDuration:0.5 delay:0.06*i options:UIViewKeyframeAnimationOptionCalculationModeLinear animations:^{
            ...
            if (i>0&&i<CARD_NUM-1) {
                //前面一张卡片
                CardViewItem * preCard = [self.disPlayCardArr objectAtIndex:i-1];
                card.transform=CGAffineTransformScale(card.transform, pow(CARD_SCALE, i), pow(CARD_SCALE, i));
...
-(void)disMissCardsComplete:(DragCardViewComplete)complete{
    
    for (int i=0; i<self.disPlayCardArr.count ;i++) {
        
        CardView *card=self.disPlayCardArr[i];
...
-(void)swipCard:(CardViewItem *)card Direction:(BOOL)isRight {
    ...
    CardViewItem * item = [self.disPlayCardArr objectAtIndex:1];
    ...
    for (int i = 0; i<CARD_NUM; i++) {
        ...
        CardViewItem * item = [self.disPlayCardArr objectAtIndex:i];
