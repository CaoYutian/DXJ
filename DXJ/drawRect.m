...
//    [self drawCircle:ctx];
    ...
    CGSize reasonBtnsize = [@"亲和材料无污染" sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12]}];
    CGFloat reasonBtnWidth = reasonBtnsize.width;
    
    //画矩形,画椭圆，多边形
    [self drawSharp:ctx];
    [self drawCircle2:ctx];
...
//        //画椭圆，如果长宽相等就是圆
//    CGContextAddEllipseInRect(ctx, CGRectMake(0, 250, 10, 10));
//    //画矩形,长宽相等就是正方形
//    CGContextAddRect(ctx, CGRectMake(70, 250, 50, 50));
    ...
    CGPathAddLineToPoint(path, &CGAffineTransformIdentity, 220, 200);
    CGPathAddLineToPoint(path, &CGAffineTransformIdentity, 236, 216);
    CGPathAddLineToPoint(path, &CGAffineTransformIdentity, 220, 232);
    CGPathAddLineToPoint(path, &CGAffineTransformIdentity, 20, 232);
...
//画圆、圆弧
-(void)drawCircle2:(CGContextRef)ctx {
     ...
     */
    //圆
    CGContextAddArc (ctx, 220, 216, 5, 0, M_PI* 2 , 0);
    CGContextFillPath(ctx);
